...
impl Counts{
    fn new(string: &[&str]) -> Self{
        ...
        let c = string.iter().map(|x| x.parse::<u32>().unwrap()).collect::<Vec<u32>>();
        Counts{
            spliced: c[0],
            unspliced: c[1],
            clipped: c[2],
...
impl Junction{
    ...
    fn dump(&self) -> String{
        format!("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}", self.contig, self.gene, self.transcript,
                                        self.intron_number, self.donnor, self.acceptor, 
                                        self.strand, self.ambiguous, self.counts.dump())
    }
    ...
    fn new(string: &Vec<&str>, header: &HashMap<String, usize>) -> Self{
        ...
        let exon_n: u16 = string[utils::get_header!("exon_number", header)].split("_").collect::<Vec<&str>>()[1].parse::<u16>().expect("unable to parse exon number junction"); 

        let pos = if exon_type == "Donnor" {string[utils::get_header!("pos", header)]}
            else{
                string[utils::get_header!("next", header)] 
            };
        let next = if exon_type == "Donnor" {string[utils::get_header!("next", header)]}
        else{
        ...
        Junction { 
            ...
            ambiguous: if string[utils::get_header!("ambiguous", header)] == "true" {true}
                       else{false},
            intron_number: if exon_type == "Donnor"{exon_n} else { exon_n - 1},
...
pub fn junction_file_from_table(table_file: &str, junction_file: &str){
    ...
    let mut out_file_open = File::create_new(junction_file.clone())//presorted out_file.clone()
    ...
    let _ = reader.read_line(&mut header_line).expect("fail to read table file for junction");
    ...
    for l in reader.lines(){
        ...
        exon_n = spt[utils::get_header!("exon_number", header)].split("_").collect::<Vec<&str>>()[1].parse::<u16>().expect("unable to parse exon number junction"); 
        intron_n = if exon_type == "Donnor"{exon_n} else { exon_n - 1};

        key = format!("{}_{}_{}", spt[utils::get_header!("gene_name", header)],
