     ...
     fn parse_read(
        ...
        match(SplicingEvent::from_read_assign(start_map, self.contig.clone(), valid_junction, self.start, self.end, &self.strand, read_strand), 
            SplicingEvent::from_read_assign(end_map, self.contig.clone(), valid_junction, self.start, self.end, &self.strand, read_strand)
                ...
                (Some(x), None) => TreeDataIntron::update_splicing_counter(&mut self.counter_intron,Some(x)),
                (None, Some(x)) => TreeDataIntron::update_splicing_counter(&mut self.counter_intron,Some(x)),
                (Some(SplicingEvent::Spliced), Some(SplicingEvent::Spliced)) => TreeDataIntron::update_splicing_counter(&mut self.counter_intron,Some(SplicingEvent::Spliced)),
                (Some(left), Some(right)) => {
                    TreeDataIntron::update_splicing_counter(&mut self.counter_intron,SplicingEvent::merge(Some(left), Some(right)))
                },
                (None, None) => (),
                (_, _) => ()
...
pub fn interval_tree_from_gtfmap(gtf_map: &HashMap<String, HashMap<String, Vec<Exon>>>) 
    ...
    let mut results: HashMap<String, interval_tree::IntervalTree<i64, TreeDataIntron>> = HashMap::new();
...
pub fn update_tree_from_bam(hash_tree: &mut HashMap<String, interval_tree::IntervalTree<i64, TreeDataIntron>>,
    bam_file: &str,
    // required parameter
